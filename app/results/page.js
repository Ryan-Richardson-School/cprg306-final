...
export default function Results() {
  ...
  return (
    <main className="p-10">
      ...
      {/* States */}
      {loading && (
        <p className="rainbow-text text-[30px]">Loading...</p>
      )}

      {error && (
        <p className="text-red-400 text-[30px]">{error}</p>
      )}
        ...
        {results.map((movie) => (
          ...
          >
            ...
            <button
              ...
              className="border-2 rounded-md px-4 py-1 rainbow-text hover:scale-105 transition"
            >
              Add to Watchlist
            </button>
          </div>
